...
declare -a SERVICES=(
  ...
  "sub2api|/opt/sub2api|Sub2API|data,postgres_data,redis_data,docker-compose.yml,.env,config.yaml"
...
find_available_tcp_port() {
  local port="${1:-8080}" max_port="${2:-8999}"
...
is_running() {
  ...
  local statuses
  statuses=$(cd "$1" && $COMPOSE_CMD ps -a --format "{{.Status}}" 2>/dev/null || true)
  [ -n "$statuses" ] || return 1
  ! echo "$statuses" | grep -qv '^Up'
}

is_restarting() {
  [ -n "$COMPOSE_CMD" ] || return 1
  (cd "$1" && $COMPOSE_CMD ps -a --format "{{.Status}}" 2>/dev/null | grep -q "Restarting")
}
...
wait_for_container_health() {
  local container="$1" timeout_seconds="${2:-120}"
...
verify_service_ready() {
  ...
  case "$name" in
    sub2api)
      print_info "等待 Sub2API 完成数据库迁移和健康检查..."
      if wait_for_container_health "sub2api" 120; then
        return 0
      fi
      print_error "$display 未在 120 秒内进入健康状态。"
      compose_in "$dir" logs --tail 50 sub2api 2>/dev/null || true
      return 1
      ;;
    *) return 0 ;;
  esac
...
AUTOGEN_EOF
    ...
    sub2api)
      if [ ! -f "$dir/docker-compose.yml" ]; then
        print_info "生成官方兼容的本地目录版 docker-compose.yml ..."
        cat > "$dir/docker-compose.yml" <<'AUTOGEN_EOF'
# Sub2API Docker Compose (auto-generated by server-manager)
...
AUTOGEN_EOF
      ...
      if [ ! -f "$dir/.env" ]; then
        print_info "生成 Sub2API 安全凭据和 .env ..."
        local server_port bind_host admin_email admin_password sub2api_image
        local postgres_password redis_password jwt_secret totp_encryption_key

        bind_host="${SUB2API_BIND_HOST:-0.0.0.0}"
        if ! valid_ipv4 "$bind_host"; then
          print_warning "SUB2API_BIND_HOST=$bind_host 不是有效 IPv4 地址，已改用 0.0.0.0。"
          bind_host="0.0.0.0"
        fi
        server_port="${SUB2API_PORT:-}"
        if [ -n "$server_port" ]; then
          if ! [[ "$server_port" =~ ^[0-9]+$ ]] || [ "$server_port" -lt 1 ] || [ "$server_port" -gt 65535 ]; then
            print_warning "SUB2API_PORT=$server_port 无效，将自动选择空闲端口。"
            server_port=""
          elif tcp_port_in_use "$server_port"; then
            print_warning "SUB2API_PORT=$server_port 已被占用，将自动选择空闲端口。"
            server_port=""
          fi
        fi
        if [ -z "$server_port" ]; then
          server_port=$(find_available_tcp_port 8080 8999) || {
            print_error "8080-8999 范围内没有可用端口。"
            return 1
          }
        fi
        ...
        if ! [[ "$admin_email" =~ ^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+$ ]]; then
          print_warning "SUB2API_ADMIN_EMAIL 格式无效，已改用 admin@sub2api.local。"
          admin_email="admin@sub2api.local"
        fi
        admin_password="${SUB2API_ADMIN_PASSWORD:-$(_random_secret)}"
        if [ "${#admin_password}" -lt 8 ] || ! [[ "$admin_password" =~ ^[A-Za-z0-9._~!@%+=,:-]+$ ]]; then
          print_warning "SUB2API_ADMIN_PASSWORD 过短或含 .env 不安全字符，已改用随机密码。"
          admin_password=$(_random_secret)
        fi
        sub2api_image="${SUB2API_IMAGE:-weishaw/sub2api:latest}"
        if ! [[ "$sub2api_image" =~ ^[A-Za-z0-9._/@:-]+$ ]]; then
          print_warning "SUB2API_IMAGE 格式无效，已改用 weishaw/sub2api:latest。"
        ...
        totp_encryption_key=$(_random_secret)

        cat > "$dir/.env" <<AUTOGEN_EOF
# Sub2API Environment (auto-generated by server-manager)
...
AUTOGEN_EOF
        chmod 600 "$dir/.env"
        print_success "已生成 .env，Web 端口为 ${server_port}。"
      fi

      mkdir -p "$dir/data" "$dir/postgres_data" "$dir/redis_data"
      ;;
...
print_install_info() {
  ...
  case "$name" in
    ...
    axonhub)
      ...
      ;;
    sub2api)
      local server_port="8080" admin_email="admin@sub2api.local" admin_password=""
      if [ -f "$dir/.env" ]; then
        server_port=$(sed -n 's/^SERVER_PORT=//p' "$dir/.env" | head -1)
        admin_email=$(sed -n 's/^ADMIN_EMAIL=//p' "$dir/.env" | head -1)
        admin_password=$(sed -n 's/^ADMIN_PASSWORD=//p' "$dir/.env" | head -1)
      fi
      print_access_info "${server_port:-8080}" "$lan_ip" "$public_ip"
      echo "  环境文件: $dir/.env"
      echo "  数据目录: $dir/data"
      echo "  管理员邮箱: ${admin_email:-admin@sub2api.local}"
      [ -n "$admin_password" ] && echo "  管理员密码: $admin_password"
      ;;
    9router)
...
do_install() {
  ...
  if start_and_verify "$dir" "$display" && verify_service_ready "$name" "$dir" "$display"; then
...
do_update() {
  ...
  compose_images=$(compose_in "$dir" config --images 2>/dev/null | sort -u || true)
