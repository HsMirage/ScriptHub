...
[ -z "$COMPOSE_CMD" ] && print_warning "未检测到 Docker Compose；Docker 服务管理不可用，但 Cockpit Tools 可继续管理。"
...
# ===== Cockpit Tools Definitions =====
COCKPIT_DIR="/opt/cockpit"
COCKPIT_APPIMAGE="$COCKPIT_DIR/Cockpit.Tools.AppImage"
COCKPIT_META="$COCKPIT_DIR/version"
COCKPIT_LOG="/var/log/cockpit-tools.log"
COCKPIT_REPO="jlcodes99/cockpit-tools"
COCKPIT_API="https://api.github.com/repos/${COCKPIT_REPO}/releases/latest"
...
_extract_db_dsn_values() {
  ...
  for file in "$dir/docker-compose.yml" "$dir/.env" "$dir/config.yml" "$dir/config.yaml"; do
    ...
    while IFS= read -r line || [ -n "$line" ]; do
      ...
      if [[ "$line" =~ ^[[:space:]]*([A-Z_]*DB_DSN|SQL_DSN|dsn)[[:space:]]*[:=][[:space:]]*(.*)$ ]]; then
...
generate_default_configs() {
  ...
  case "$name" in
    postgres|newapi|metapi|axonhub)
      check_pg_credentials || return 1
      ;;
  esac
...
cockpit_is_installed() {
  [ -x "$COCKPIT_APPIMAGE" ]
}

cockpit_pids() {
  pgrep -f "$COCKPIT_APPIMAGE" 2>/dev/null || true
}
...
cockpit_latest_info() {
  local arch json tag url
  arch=$(cockpit_arch)
  if [ -z "$arch" ]; then
    print_error "当前 CPU 架构 $(uname -m) 暂未匹配到 Cockpit Tools 的 Linux AppImage。"
    return 1
  fi

  if ! command -v curl >/dev/null 2>&1; then
    print_error "未安装 curl，无法从 GitHub 下载 Cockpit Tools。"
    return 1
  fi

  json=$(curl -fsSL "$COCKPIT_API") || {
    print_error "无法读取 GitHub 最新版本信息，请检查网络。"
    return 1
  }
  ...
  url=$(printf '%s\n' "$json" \
    ...
    | grep 'AppImage' \
    | grep -v '\.sig"' \
    | grep "_${arch}\.AppImage" \
    | head -1 \
    | sed -E 's/.*"browser_download_url"[[:space:]]*:[[:space:]]*"([^"]+)".*/\1/')

  if [ -z "$tag" ] || [ -z "$url" ]; then
    print_error "没有在最新 Release 中找到适合 $arch 的 AppImage。"
    return 1
  fi
...
cockpit_install_or_update() {
  local mode="$1"
  local info tag url tmp_file

  info=$(cockpit_latest_info) || return 1
  tag=$(echo "$info" | cut -d'|' -f1)
  url=$(echo "$info" | cut -d'|' -f2-)

  mkdir -p "$COCKPIT_DIR"
  tmp_file="$COCKPIT_DIR/Cockpit.Tools.AppImage.tmp"

  print_info "下载 Cockpit Tools $tag ..."
  ...
  chmod +x "$tmp_file"
  mv "$tmp_file" "$COCKPIT_APPIMAGE"
  printf '%s\n%s\n' "$tag" "$url" > "$COCKPIT_META"

  if [ "$mode" = "update" ]; then
    print_success "Cockpit Tools 已更新到 $tag。"
  else
    print_success "Cockpit Tools 已安装到 $COCKPIT_APPIMAGE。"
  fi
}

do_cockpit_install() {
  print_header "安装 Cockpit Tools"
  if cockpit_is_installed; then
    print_warning "Cockpit Tools 已安装。如需更新请选择"更新"。"
    return
  fi
  cockpit_install_or_update "install"
}

do_cockpit_update() {
  print_header "更新 Cockpit Tools"
  if cockpit_is_running; then
    print_warning "Cockpit Tools 正在运行，更新前会先停止。"
    do_cockpit_stop
  fi
  cockpit_install_or_update "update"
}

do_cockpit_start() {
  print_header "启动 Cockpit Tools"
  if ! cockpit_is_installed; then
    print_error "Cockpit Tools 未安装。"; return
  fi
  if cockpit_is_running; then
    print_warning "Cockpit Tools 已在运行。"; return
  fi
  print_info "正在后台启动，日志写入 $COCKPIT_LOG ..."
  nohup "$COCKPIT_APPIMAGE" >> "$COCKPIT_LOG" 2>&1 &
  sleep 3
  if cockpit_is_running; then
    print_success "Cockpit Tools 已启动。"
  else
    print_error "Cockpit Tools 未能保持运行。服务器如果没有图形桌面环境，AppImage 可能无法显示窗口。"
    [ -f "$COCKPIT_LOG" ] && tail -n 30 "$COCKPIT_LOG" || true
  fi
}

do_cockpit_stop() {
  print_header "停止 Cockpit Tools"
  local pids
  pids=$(cockpit_pids)
  if [ -z "$pids" ]; then
    print_warning "Cockpit Tools 未运行。"; return
  fi
  echo "$pids" | xargs kill 2>/dev/null || true
  sleep 2
  pids=$(cockpit_pids)
  [ -n "$pids" ] && echo "$pids" | xargs kill -9 2>/dev/null || true
  print_success "Cockpit Tools 已停止。"
}

do_cockpit_restart() {
  print_header "重启 Cockpit Tools"
  do_cockpit_stop
  do_cockpit_start
}

do_cockpit_status() {
  print_header "Cockpit Tools 状态"
  echo ""
  if cockpit_is_installed; then
    print_success "已安装: $COCKPIT_APPIMAGE"
    if [ -f "$COCKPIT_META" ]; then
      echo "版本信息:"; sed -n '1,2p' "$COCKPIT_META"
    fi
  else
    print_error "未安装: $COCKPIT_APPIMAGE"
  fi
  local pids
  pids=$(cockpit_pids)
  if [ -n "$pids" ]; then
    print_success "运行中，PID: $(echo "$pids" | tr '\n' ' ' | xargs)"
  else
    print_warning "未运行。"
  fi
  [ -f "$COCKPIT_LOG" ] && echo "日志: $COCKPIT_LOG"
}

do_cockpit_logs() {
  print_header "Cockpit Tools 日志 (最近 100 行)"
  if [ ! -f "$COCKPIT_LOG" ]; then
    print_warning "暂无日志文件: $COCKPIT_LOG"; return
  fi
  tail -n 100 "$COCKPIT_LOG"
}

do_cockpit_uninstall() {
  print_header "卸载 Cockpit Tools"
  if ! cockpit_is_installed && [ ! -d "$COCKPIT_DIR" ]; then
    print_error "Cockpit Tools 未安装。"; return
  fi
  local confirm=""
  read -r -p "确认删除 $COCKPIT_DIR 中的 Cockpit Tools？[y/N]: " confirm < /dev/tty
  if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
    print_info "已取消卸载。"; return
  fi
  do_cockpit_stop
  rm -rf "$COCKPIT_DIR"
  print_success "Cockpit Tools 已卸载。"
}
...
show_cockpit_menu() {
  while true; do
    local status_icon=""
    if cockpit_is_installed; then
      cockpit_is_running && status_icon="${GREEN}● 运行中${NC}" || status_icon="${YELLOW}● 已停止${NC}"
    else
      status_icon="${RED}○ 未安装${NC}"
    fi
    echo ""
    echo "=============================================="
    echo -e "    Cockpit Tools  $status_icon"
    echo "=============================================="
    echo "  1) 安装到 /opt/cockpit"
    echo "  2) 更新到 GitHub 最新版"
    echo "  3) 启动"
    echo "  4) 重启"
    echo "  5) 停止"
    echo "  6) 查看状态"
    ...
    case "$choice" in
      1) do_cockpit_install ;;  2) do_cockpit_update ;;
      3) do_cockpit_start   ;;  4) do_cockpit_restart ;;
      5) do_cockpit_stop    ;;  6) do_cockpit_status ;;
      7) do_cockpit_logs    ;;  9) do_cockpit_uninstall ;;
      0|q|Q) return ;;
...
show_main_menu() {
  while true; do
    ...
    if cockpit_is_installed; then
      cockpit_is_running && cockpit_status="${GREEN}运行中${NC}" || cockpit_status="${YELLOW}已停止${NC}"
    else
      cockpit_status="${RED}未安装${NC}"
    fi
    local cockpit_choice="$i"
    echo -e "  $cockpit_choice) Cockpit Tools  [$cockpit_status]"
    ...
    case "$choice" in
      ...
      [1-9]|[1-9][0-9])
        ...
        if [ "$choice" -eq "$cockpit_choice" ]; then
          show_cockpit_menu; continue
        fi
        local idx=$((choice - 1))
